...
namespace CT_Win.UserControls
{
    public partial class PacienteUserControl : UserControl
    {
        ...
        public PacienteUserControl(PrincipalForm principal)
        {
        ...
        private void CarregarGrid(string filtro = "")
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filtro))
                {
                    ...
                    lista = lista.Where(p =>
                        (p.NomeExibicao ?? "").ToLowerInvariant().Contains(f) ||
                        (p.CPF ?? "").Contains(f) ||
                        (p.Cidade?.Nome ?? "").ToLowerInvariant().Contains(f)
                    ).ToList();
                }
        ...
        private void PreencherGrid(List<Paciente> lista)
        {
            dgv.Rows.Clear();
            foreach (var p in lista)
            {
                string cpf      = FormatarCPF(p.CPF);
                string cidade   = p.Cidade != null ? p.Cidade.Nome + "/" + p.Cidade.UF : "—";
                string nivel    = p.NivelSuporte.HasValue ? "N" + p.NivelSuporte.Value : "—";
                string idade    = p.Idade >= 0 ? p.Idade + " anos" : "—";
                string telefone = FormatarTelefone(p.Telefone);
                string status   = p.Ativo ? "Ativo" : "Inativo";

                int idx = dgv.Rows.Add(p.NomeExibicao, cpf, cidade, nivel, idade, telefone, status);
                dgv.Rows[idx].Tag = p;
            }
        }

        private void txtBusca_TextChanged(object sender, EventArgs e) => CarregarGrid(txtBusca.Text);
        ...
        private void dgv_SelectionChanged(object sender, EventArgs e)
        {
            ...
            btnObjetivos.Enabled = temSelecao;
        ...
        private void dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var pac = dgv.Rows[e.RowIndex].Tag as Paciente;
            if (pac != null) AbrirEdicao(pac.ID);
        }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            var pac = dgv.SelectedRows[0].Tag as Paciente;
            if (pac != null) AbrirEdicao(pac.ID);
        }
        ...
        private void AbrirEdicao(int id)
        {
            try
            {
                ...
                using (var form = new PacienteForm(p))
                {
                    if (form.ShowDialog(this) == DialogResult.OK)
                        CarregarGrid(txtBusca.Text);
                }
            ...
            {
                MessageBox.Show("Erro ao abrir paciente: " + ex.Message,
                    "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnInativar_Click(object sender, EventArgs e)
        {
            ...
            var resp = MessageBox.Show(
                "Deseja " + acao + " o paciente \"" + nome + "\"?\n\n" +
                "Os dados serão preservados (histórico, evoluções).",
            ...
            try
            {
                if (ativo) _dao.Excluir(pac.ID);
                else       _dao.Reativar(pac.ID);
                CarregarGrid(txtBusca.Text);
            }
            catch (Exception ex)
            {
        ...
        private void btnObjetivos_Click(object sender, EventArgs e)
        {
            ...
            using (var form = new ObjetivosForm(pac))
        ...
        {
            btnNavPacientes.BackColor = System.Drawing.Color.FromArgb(44, 95, 127);
            btnNavPacientes.ForeColor = System.Drawing.Color.White;
            btnNavPacientes.Font      = new System.Drawing.Font("Segoe UI", 9F, System.Drawing.FontStyle.Bold);
