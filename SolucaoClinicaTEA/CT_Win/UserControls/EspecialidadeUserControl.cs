...
{
    public partial class EspecialidadeUserControl : UserControl
    {
        ...
        public EspecialidadeUserControl(PrincipalForm principal)
        {
        ...
        private void CarregarGrid(string filtro = "")
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(filtro))
                {
                    ...
                    lista = lista.Where(e =>
                        (e.Nome          ?? "").ToLowerInvariant().Contains(f) ||
                        (e.ConselhoSigla ?? "").ToLowerInvariant().Contains(f)
                    ).ToList();
                }
        ...
        private void PreencherGrid(List<Especialidade> lista)
        {
            dgv.Rows.Clear();
            foreach (var e in lista)
            {
                string conselho = string.IsNullOrWhiteSpace(e.ConselhoSigla) ? "—" : e.ConselhoSigla;
                string status   = e.Ativo ? "Ativo" : "Inativo";

                int idx = dgv.Rows.Add(e.Nome, conselho, status);
                dgv.Rows[idx].Tag = e;
            }
        }
        ...
        private void dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var esp = dgv.Rows[e.RowIndex].Tag as Especialidade;
            if (esp != null) AbrirEdicao(esp.ID);
        }
        ...
        private void dgv_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            // Especialidade nome chip (colNome)
            if (e.ColumnIndex == colNome.Index)
            {
                var esp = dgv.Rows[e.RowIndex].Tag as Especialidade;
                if (esp != null && !string.IsNullOrEmpty(esp.Nome))
                    DesenharChip(e.Graphics, e.CellBounds, esp.Nome,
                        PastelDe(esp.CorHex), EscurecerHex(esp.CorHex));
                else
        ...
        {
            using (var form = new EspecialidadeForm())
            {
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            var esp = dgv.SelectedRows[0].Tag as Especialidade;
            if (esp != null) AbrirEdicao(esp.ID);
        }
        ...
        private void AbrirEdicao(int id)
        {
            try
            {
                var esp = _dao.Buscar(id);
                if (esp == null) return;

                using (var form = new EspecialidadeForm(esp))
                {
                    if (form.ShowDialog(this) == DialogResult.OK)
                        CarregarGrid(txtBusca.Text);
                }
            ...
            {
                MessageBox.Show("Erro ao abrir especialidade: " + ex.Message,
                    "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnInativar_Click(object sender, EventArgs e)
        {
            ...
            var esp  = row.Tag as Especialidade;
            if (esp == null) return;
            ...
            var resp = MessageBox.Show(
                "Deseja " + acao + " a especialidade \"" + nome + "\"?",
            ...
            try
            {
                if (ativo) _dao.Excluir(esp.ID);
                else       _dao.Reativar(esp.ID);
                CarregarGrid(txtBusca.Text);
            }
            catch (Exception ex)
            {
        ...
        {
            btnNavEspecialidades.BackColor = System.Drawing.Color.FromArgb(44, 95, 127);
            btnNavEspecialidades.ForeColor = System.Drawing.Color.White;
            btnNavEspecialidades.Font      = new System.Drawing.Font("Segoe UI", 9F, System.Drawing.FontStyle.Bold);
